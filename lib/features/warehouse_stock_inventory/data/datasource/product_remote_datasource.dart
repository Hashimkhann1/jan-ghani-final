...
// Products + Inventory + Audit Log DB operations
// =============================================================
...
class ProductRemoteDataSource {
    ...
    final productResult = await conn.execute(
      Sql.named('''
        INSERT INTO products (
          ...
          @categoryId, @unitOfMeasure, @costPrice, @sellingPrice,
    ...
    await conn.execute(
      Sql.named('''
        UPDATE products SET
          ...
          wholesale_price = @wholesalePrice, tax_rate      = @taxRate,
          min_stock_level = @minStockLevel, max_stock_level = @maxStockLevel,
          reorder_point   = @reorderPoint,
          is_active       = @isActive,    is_track_stock  = @isTrackStock
        WHERE id = @id
      '''),
      parameters: {
        'id': newProduct.id, 'sku': newProduct.sku, 'barcode': newProduct.barcode,
        'name': newProduct.name, 'description': newProduct.description,
        'categoryId': newProduct.categoryId, 'unitOfMeasure': newProduct.unitOfMeasure,
        'costPrice': newProduct.costPrice, 'sellingPrice': newProduct.sellingPrice,
        'wholesalePrice': newProduct.wholesalePrice, 'taxRate': newProduct.taxRate,
        'minStockLevel': newProduct.minStockLevel, 'maxStockLevel': newProduct.maxStockLevel,
        'reorderPoint': newProduct.reorderPoint, 'isActive': newProduct.isActive,
  ...
  Future<void> delete({
    ...
    final conn = await _db;
    await conn.execute(
      Sql.named('UPDATE products SET deleted_at = NOW() WHERE id = @id'),
      parameters: {'id': id},
    );
    await _insertAuditLog(
  ...
  // ── SKU EXISTS ────────────────────────────────────────────
  Future<bool> skuExists(String sku, String warehouseId, {String? excludeId}) async {
    final conn = await _db;
    final result = await conn.execute(
      Sql.named('''
        ...
        LIMIT 1
      '''),
      parameters: {'sku': sku, 'warehouseId': warehouseId, if (excludeId != null) 'excludeId': excludeId},
    );
  ...
  Future<void> _insertAuditLog({
    required Connection conn, required String productId,
  ...
  Map<String, dynamic> _toMap(ResultRow row) {
    ...
    return {
      ...
      'sku': m['sku']?.toString() ?? '', 'barcode': m['barcode']?.toString(),
      'name': m['name']?.toString() ?? '', 'description': m['description']?.toString(),
      'category_id': m['category_id']?.toString(), 'category_name': m['category_name']?.toString(),
      ...
      'min_stock_level': m['min_stock_level'], 'max_stock_level': m['max_stock_level'],
      'reorder_point': m['reorder_point'],
      'is_active': m['is_active'] ?? true, 'is_track_stock': m['is_track_stock'] ?? true,
      'created_at': m['created_at'], 'updated_at': m['updated_at'], 'deleted_at': m['deleted_at'],
      'quantity': m['quantity'], 'reserved_quantity': m['reserved_quantity'],
    };
  }
}
