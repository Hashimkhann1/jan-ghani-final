...
class AccountantCustomerReportState {
  ...
  final String?                             filterType; // null=All | 'credit' | 'petrol' | 'exceeded'
...
class AccountantCustomerReportNotifier
  ...
  List<AccountantCustomerReportModel> _applyFilters(
    ...
    if (type == 'exceeded') {
      list = list.where((c) => c.isCreditLimitExceeded).toList();
    } else if (type != null) {
  ...
  AccountantCustomerReportSummary _buildSummary(
      ...
      AccountantCustomerReportSummary(
        ...
        limitExceededCount: customers.where((c) => c.isCreditLimitExceeded).length,
      );
}

// ── Family Provider ───────────────────────────────────────
final accountantCustomerReportProvider = StateNotifierProvider.autoDispose
    .family<AccountantCustomerReportNotifier, AccountantCustomerReportState, String>(
      (ref, branchId) {
