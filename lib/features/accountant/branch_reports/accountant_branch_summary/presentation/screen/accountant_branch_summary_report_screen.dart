...
import '../provider/accountant_branch_summary_provider.dart';

class BranchSummaryReportScreen extends ConsumerStatefulWidget {
  const BranchSummaryReportScreen({required this.branchId, super.key});
  final String branchId;

  @override
  ConsumerState<BranchSummaryReportScreen> createState() =>
      _BranchSummaryReportScreenState();
}

class _BranchSummaryReportScreenState
    extends ConsumerState<BranchSummaryReportScreen> {
  ...
  Future<void> _pickDate(BuildContext context, bool isFrom) async {
    final state  = ref.read(branchSummaryProvider(widget.branchId));
    ...
    if (picked != null) {
      final n = ref.read(branchSummaryProvider(widget.branchId).notifier);
      if (isFrom) {
  ...
  Widget build(BuildContext context) {
    final state    = ref.watch(branchSummaryProvider(widget.branchId));
    final notifier = ref.read(branchSummaryProvider(widget.branchId).notifier);

    ref.listen<BranchSummaryState>(
        branchSummaryProvider(widget.branchId), (prev, next) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Branch Summary Report',
            style: TextStyle(
                fontSize:   17,
            ...
            Padding(
              ...
              child: Column(children: [
                ...
                _SummaryRow(
                  ...
                  label: 'Expense',
                  value: fmtAmt(summary.totalExpense),
            ...
            child: Row(children: [
              ...
              Expanded(
                child: Wrap(spacing: 4, runSpacing: 4, children: [
                  if (d.totalCashSale > 0)
                    _MiniChip(
                        label: 'Cash: ${widget.fmtAmt(d.totalCashSale)}',
                        color: AppColor.success),
                  if (d.totalCardSale > 0)
                    _MiniChip(
                        label: 'Card: ${widget.fmtAmt(d.totalCardSale)}',
                        color: AppColor.info),
                  if (d.totalCreditSale > 0)
                    _MiniChip(
                        label: 'Credit: ${widget.fmtAmt(d.totalCreditSale)}',
                        color: AppColor.warning),
                  if (d.totalInstallment > 0)
                    _MiniChip(
                        label: 'Install: ${widget.fmtAmt(d.totalInstallment)}',
                        color: AppColor.primary),
                ]),
              ),
              const SizedBox(width: 8),
          ...
          Padding(
            ...
            child: Column(children: [
              ...
              _DetailRow(
                  icon:  Icons.payments_outlined,
                  label: 'Cash Sale',
                  value: widget.fmtAmt(d.totalCashSale),
                  color: AppColor.success),
              const SizedBox(height: 8),
              _DetailRow(
                  icon:  Icons.credit_card_outlined,
                  label: 'Card Sale',
                  value: widget.fmtAmt(d.totalCardSale),
                  color: AppColor.info),
              const SizedBox(height: 8),
              _DetailRow(
                  icon:  Icons.receipt_long_outlined,
                  label: 'Credit Sale',
                  value: widget.fmtAmt(d.totalCreditSale),
                  color: AppColor.warning),
              const SizedBox(height: 8),
              _DetailRow(
                  icon:  Icons.calendar_month_outlined,
                  label: 'Installment',
                  value: widget.fmtAmt(d.totalInstallment),
                  color: AppColor.primary),
              ...
              _DetailRow(
                  icon:  Icons.south_rounded,
                  label: 'Cash In',
                  value: widget.fmtAmt(d.totalCashIn),
                  color: AppColor.success),
              const SizedBox(height: 8),
              _DetailRow(
                  icon:  Icons.north_rounded,
                  label: 'Cash Out',
                  value: widget.fmtAmt(d.totalCashOut),
                  color: AppColor.error),
              const SizedBox(height: 8),
              _DetailRow(
                  icon:  Icons.money_off_outlined,
                  label: 'Expense',
                  value: widget.fmtAmt(d.totalExpense),
                  color: const Color(0xFFEF4444)),
