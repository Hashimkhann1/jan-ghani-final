...
class ProductModel {
  ...
  final String   sku;
  final List<String> barcodes;   // ← was: String? barcode
  final String   name;
  ...
  const ProductModel({
    ...
    required this.sku,
    this.barcodes = const [],   // ← default empty list
    required this.name,
    this.description,
    this.categoryId,
    this.categoryName,
  ...
  factory ProductModel.fromMap(Map<String, dynamic> m) {
    return ProductModel(
      ...
      sku:            m['sku']?.toString()             ?? '',
      barcodes:       _toBarcodeList(m['barcode']),    // ← postgres array parse
      ...
      wholesalePrice: m['wholesale_price'] != null
          ? _toDouble(m['wholesale_price']) : null,
  ...
  bool get needsReorder => isTrackStock && quantity <= reorderPoint;

  // postgres text[] → Dart List<String>
  // postgres driver yeh 3 formats mein de sakta hai:
  //   1. Already a List  → ['897422342', '123456']
  //   2. String literal  → '{897422342,123456}'
  //   3. null / empty
  static List<String> _toBarcodeList(dynamic v) {
    ...
    // Case 2: Postgres text string '{val1,val2}'
    final str = v.toString().trim();
    if (str.isEmpty || str == '{}') return [];

    // Strip curly braces then split by comma
