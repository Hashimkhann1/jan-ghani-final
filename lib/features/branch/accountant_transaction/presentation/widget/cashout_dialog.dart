...
import '../../domain/entity/accountant_user_entity.dart'; // ✅ model → entity
...
class _CashOutDialogState extends ConsumerState<CashOutDialog> {
  ...
  AccountantUserEntity? _selected; // ✅ AccountantUserModel → AccountantUserEntity
  bool _isSaving = false;

  double get _enteredAmount =>
      double.tryParse(_amountCtrl.text.trim()) ?? 0.0;

  double get _branchBalance =>
      ref.read(branchTotalAmountProvider(widget.branchId)).asData?.value ??
          0.0;
  ...
  Widget build(BuildContext context) {
    ...
    final branchAmountAsync =
    ref.watch(branchTotalAmountProvider(widget.branchId));
            ...
            child: Column(
              ...
              children: [
                ...
                accountantsAsync.when(
                  ...
                  // ✅ data type is now List<AccountantUserEntity>
                  data: (accountants) =>
                      AppSearchableDropdown<AccountantUserEntity>(
                        fullWidth: true,
                        value: _selected,
                        hint: 'Select accountant',
                        prefixIcon: Icons.person_outline_rounded,
                        items: accountants
                            .map((acc) => DropdownItem<AccountantUserEntity>(
                          value: acc,
                          label: acc.name,
