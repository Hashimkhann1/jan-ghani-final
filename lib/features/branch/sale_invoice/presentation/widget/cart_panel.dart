...
class _ReturnItemRowState extends ConsumerState<_ReturnItemRow> {
  ...
  void didUpdateWidget(_ReturnItemRow old) {
    ...
    // price readOnly hai — sirf sync karo, focus guard ki zaroorat nahi
    final p = _fmtD(widget.item.returnPrice);
    if (_priceCtrl.text != p) _priceCtrl.text = p;
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(flex: 2, child: _RTF(
          ...
          onFocusChange: (_) {},      // kuch nahi
          onChanged:     (_) {},      // kuch nahi
          onSubmitted:   (_) {},      // kuch nahi
        )),
...
class _RTF extends StatefulWidget {
  ...
  const _RTF({
    ...
    this.prefix,
    this.color    = AppColor.textPrimary,
    this.readOnly = false,              // default false — baaki sab same
  });
...
class _RTFState extends State<_RTF> {
  ...
  void initState() {
    ...
    _focus = FocusNode()
      ..addListener(() {
        widget.onFocusChange(_focus.hasFocus);
        // readOnly ho toh select-all ki zaroorat nahi
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: TextField(
      ...
      inputFormatters: widget.readOnly
          ? []   // readOnly mein formatter nahi chahiye
          : [FilteringTextInputFormatter.allow(RegExp(r'^\d*\.?\d*'))],
      textAlign:    TextAlign.center,
      cursorHeight: 14,
      style: TextStyle(
          fontSize:   13,
          fontWeight: FontWeight.w700,
          // ✅ readOnly ho toh text grey — visually disabled dikhega
          color: widget.readOnly ? AppColor.textHint : widget.color),
      decoration: InputDecoration(
        prefixText:  widget.prefix != null ? '${widget.prefix} ' : null,
        prefixStyle: const TextStyle(fontSize: 10, color: AppColor.textHint),
        isDense:     true,
        filled:      true,
        // ✅ readOnly ho toh grey background
        ...
        border:        InputBorder.none,
        enabledBorder: InputBorder.none,
        // ✅ readOnly ho toh focus border bhi nahi
