// lib/features/branch/sale_invoice/presentation/screen/cart_panel.dart
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        // Row 2: Type + Customer  (RefundType REMOVED)
        Row(children: [
          Expanded(
            flex: 2,
    ...
    return Column(
      ...
      children: [
        ...
        DropdownSearch<CustomerModel?>(
          ...
          itemAsString: (c) => c == null ? 'Walk In' : '${c.name} — ${c.code}',
          onSelected: onSelected,
          decoratorProps: DropDownDecoratorProps(
            decoration: InputDecoration(
              hintText: 'Customer (optional)',
              hintStyle: const TextStyle(fontSize: 13, color: AppColor.textHint),
              prefixIcon: Icon(Icons.person_outline, size: 18, color: AppColor.grey500),
              filled: true,
              fillColor: isReturn ? AppColor.error.withOpacity(0.05) : AppColor.grey100,
          ...
          popupProps: PopupProps.menu(
            ...
            itemBuilder: (ctx, customer, isDisabled, isSelected) => ListTile(
              leading: CircleAvatar(
                radius: 16,
                backgroundColor: accent.withOpacity(0.1),
                child: Text(
                  customer == null ? 'W' : customer.name[0].toUpperCase(),
              ...
              title: Text(
                ...
                style: TextStyle(
                    ...
                    color: isSelected ? accent : AppColor.textPrimary),
              ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        _RSR(label: 'Total Discount', value: '-${_fmt(returnState.totalDiscount)}', color: AppColor.success),
        const Divider(color: AppColor.grey200, height: 14),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('Grand Total',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: AppColor.error)),
            Text('Rs ${_fmt(returnState.grandTotal)}',
                style: const TextStyle(
                    fontSize: 18, fontWeight: FontWeight.w900,
                    color: AppColor.error, letterSpacing: -0.5)),
          ],
        ),
        ...
        Row(children: [
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: returnState.cartItems.isEmpty ? null : notifier.clearCart,
              icon: const Icon(Icons.clear_all, size: 16),
              label: const Text('Clear', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
...
class _ReturnItemRowState extends ConsumerState<_ReturnItemRow> {
  ...
  void initState() {
    ...
    _priceCtrl = TextEditingController(text: widget.item.returnPrice.toStringAsFixed(0));
    _disCtrl   = TextEditingController(text: widget.item.discountAmount.toStringAsFixed(0));
  }
  ...
  String _fmt(double q) => q % 1 == 0 ? q.toInt().toString() : q.toStringAsFixed(2);
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            width: 28, height: 28,
            decoration: BoxDecoration(color: AppColor.errorLight, borderRadius: BorderRadius.circular(7)),
...
class _RTFState extends State<_RTF> {
  ...
  void initState() {
    super.initState();
    _focus = FocusNode()..addListener(() => widget.onFocusChange(_focus.hasFocus));
  }
...
class _RH extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(text,
      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColor.error, letterSpacing: 0.3),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        Text(value, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: color ?? AppColor.textPrimary)),
      ],
    ),
  );
}
    ...
    child: Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: AppColor.primary.withOpacity(0.06), shape: BoxShape.circle),
          child: const Icon(Icons.shopping_cart_outlined, size: 48, color: AppColor.primary),
        ),
        const SizedBox(height: 14),
        const Text('Cart is empty',
            style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColor.textSecondary)),
  ...
  Widget build(BuildContext context) => Center(
    child: Column(
      ...
      children: [
        Container(
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(color: AppColor.error.withOpacity(0.06), shape: BoxShape.circle),
          child: const Icon(Icons.assignment_return_outlined, size: 48, color: AppColor.error),
        ),
        const SizedBox(height: 14),
