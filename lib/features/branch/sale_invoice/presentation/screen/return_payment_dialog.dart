...
class _ReturnPaymentDialogState extends ConsumerState<_ReturnPaymentDialog> {
  ...
  // ── Customer ka existing credit/odhar balance ─────────────────
  // ✅ FIX: pehle 'creditBalance' field try ho raha tha jo
  // CustomerModel mein exist nahi karta (hamesha 0.0 fallback deta
  // tha). Sale invoice dialog ki tarah 'balance' field use karo.
  double get _existingCredit =>
      ref.read(saleReturnProvider).selectedCustomer?.balance ?? 0.0;
  ...
  Future<void> _confirm() async {
    ...
    // ✅ saveReturn() se PEHLE capture karo — state clear hone se pehle
    // Return mein credit refund se customer ka odhar KAM hota hai.
    final capturedPreviousAmount = _hasCustomer ? _existingCredit : null;
    final capturedNewAmount      = _hasCustomer
        ? (_existingCredit - _credit).clamp(0.0, double.infinity)
        : null;
    // payAmount: customer ho ya walk-in, jitna cash+card actual refund hua
    final capturedPayAmount      = _cash + _card;
    ...
    final ok = await notifier.saveReturn(
      previousAmount: capturedPreviousAmount,
      newAmount:      capturedNewAmount,
      payAmount:      capturedPayAmount,
    );
    if (!mounted) return;
    ...
    if (ok) {
      ...
      if (_printReceipt) {
        ...
        await ThermalPrintService.printSaleReturn(
          ...
          payments:        payments,
          refundType:      dominant,
          previousBalance: capturedPreviousAmount,
          paidAmount:      capturedPayAmount,
          currentBalance:  capturedNewAmount,
        );
      }
    }
  }
