...
class _PaymentDialogState extends ConsumerState<_PaymentDialog> {
  ...
  Future<void> _confirm() async {
    ...
    final capturedExistingBalance = _hasCustomer  ? _existingBalance : null;
    final capturedNewBalance      = _hasCustomer  ? _newBalance      : null;
    // payAmount: customer ho ya walk-in, hamesha capture (cash+card,
    // jitna actual cashier ne enter kiya — extra payment scenario sameet)
    final capturedPayAmount       = _totalPaid;
    ...
    try {
      ...
      final success = await notifier.saveInvoice(
        previousAmount: capturedExistingBalance,
        newAmount:      capturedNewBalance,
