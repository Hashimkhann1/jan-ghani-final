...
// lib/features/branch/sale_invoice/presentation/provider/sale_invoice_provider.dart
//
// ── SIRF YEH METHOD ADD KARO SaleInvoiceNotifier CLASS MEIN ──
// Baqi poora file same rehta hai

// ─────────────────────────────────────────────────────────────
// SaleInvoiceNotifier ke andar, saveInvoice() method ke baad
// yeh naya method add karo:
// ─────────────────────────────────────────────────────────────

/*

  // ── Extra Customer Payment (KEY FIX) ──────────────────────
  // Jab registered customer zyada cash de (sale se zyada),
  // wo extra amount customer ke existing balance se minus hota hai.
  //
  // Example:
  //   existing balance = Rs 10,000
  //   sale             = Rs 1,680
  //   customer pays    = Rs 5,000
  //   extra            = Rs 3,320  ← yeh method call hota hai
  //   new balance      = Rs 6,680  ✓
  Future<void> applyExtraCustomerPayment(double extraPayment) async {
    final customer = state.selectedCustomer;
    if (customer == null || extraPayment <= 0) return;

    try {
      await _ds.applyExtraCustomerPayment(
        customerId:   customer.id,
        extraPayment: extraPayment,
      );

      // Refresh customer list so new balance shows immediately
      _ref.read(customerProvider.notifier).loadCustomers();
    } catch (e) {
      debugPrint('applyExtraCustomerPayment error: $e');
    }
  }
...
// ─────────────────────────────────────────────────────────────
// COMPLETE UPDATED FILE (with the method already added):
// ─────────────────────────────────────────────────────────────
...
class SaleInvoiceNotifier extends StateNotifier<SaleInvoiceState> {
  ...
  bool addToCartByBarcode(String query) {
    ...
    BranchStockModel? found;
    found = allProducts.cast<BranchStockModel?>().firstWhere(
          (p) => p!.barcode != null && p.barcode!.toLowerCase() == query.toLowerCase(),
      orElse: () => null,
    );
    found ??= allProducts.cast<BranchStockModel?>().firstWhere(
          (p) => p!.sku.toLowerCase() == query.toLowerCase(),
  ...
  // ── KEY FIX: Extra Customer Payment ──────────────────────
  //
  // Yeh method tab call hota hai jab registered customer
  // sale amount se ZYADA cash deta hai.
  //
  // PEHLE wali galti: direct UPDATE se balance to update
  // hota tha lekin cash counter (installment) nahi hota tha.
  //
  // AB FIX: customer_ledger mein INSERT karo
  //   Trigger 1: fn_customer_payment   → balance  -= extraPayment ✓
  //   Trigger 2: fn_ledger_to_cash_counter → installment += extraPayment ✓
  //
  // NOTE: customerId, customerName, invoiceNo dialog se
  //       PEHLE capture karke pass karo (saveInvoice ke baad
  //       state clear ho jata hai)
  Future<void> applyExtraCustomerPayment({
    ...
    if (_counterId == null || _counterId!.isEmpty) return;
    ...
    try {
      ...
      // Refresh: customer list + cash counter
      _ref.read(customerProvider.notifier).loadCustomers();
      _ref.read(cashCounterProvider.notifier).loadRecords();
    } catch (e) {
      debugPrint('applyExtraCustomerPayment error: $e');
  ...
  void clearError()   => state = state.copyWith(errorMessage: null, clearSuccess: true);
  void clearSuccess() => state = state.copyWith(clearSuccess: true);
}
