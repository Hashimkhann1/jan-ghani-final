...
class SaleReturnNotifier extends StateNotifier<SaleReturnState> {
  ...
  void selectCustomer(CustomerModel? customer) {
    ...
    if (customer == null) {
      final updated = state.payments.where((p) => p.method != 'credit')
          .toList();
  ...
  // ✅ NEW — Sale Invoice wali updateSubTotal logic, Sale Return ke liye
  // (Return mein tax nahi hota, isliye formula: qty = (subtotal + discount) / price)
  void updateSubTotal(String cartId, double newSubTotal) {
    ...
    if (item.returnPrice <= 0) return;
    final newQty = (newSubTotal + item.discountAmount) / item.returnPrice;
    if (newQty <= 0) return;
    state = state.copyWith(
      cartItems: state.cartItems
          .map((i) => i.cartId == cartId ? i.copyWith(quantity: newQty) : i)
          .toList(),
    );
    _resetPayments();
  }
  ...
  Future<bool> saveReturn({
    ...
    if (!state.isPaymentValid) {
      state = state.copyWith(
          errorMessage:
          'Payment total Rs ${state.totalPaid.toStringAsFixed(
              0)} grand total se match nahi karta');
      return false;
    }
    ...
    try {
      await _ds.saveReturn(
        ...
        refundType: state.refundTypeForDb,
